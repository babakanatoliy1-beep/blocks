...
WINDOW_HEIGHT = BOARD_OUTER + 2 * PADDING + 180

WIN_VALUE = 2048
WIN_STEPS = math.log2(WIN_VALUE)  # = 11 for 2048
...
def load_state() -> dict | None:
    """Завантажує збережений стан гри (якщо є). Повертає dict або None."""
    try:
...
def save_state(game: "Game2048") -> None:
    """Зберігає повний стан гри у файл."""
    data = {
...
def load_best_score() -> int:
    """Завантажує лише best_score (сумісність зі старими файлами)."""
    try:
        data = json.loads(SAVE_FILE.read_text(encoding="utf-8"))
...
def save_best_score(score: int) -> None:
    """Оновлює best_score у файлі (зберігає інші поля, якщо вони є)."""
    data = {}
    try:
        data = json.loads(SAVE_FILE.read_text(encoding="utf-8"))
    except Exception:
...
def main(page: ft.Page) -> None:
    ...
    status_text = ft.Text(
        "Стрілки - хід. R - заново. Z - відміна ходу",
        size=16,
        color=theme()["txt"],
        text_align=ft.TextAlign.CENTER,
    )

    # Progress bar showing progress of the largest tile toward WIN_VALUE
    ...
    def refresh_ui(msg: str = "") -> None:
        # знайти максимальне значення на полі (може бути 0)
        ...
        for r in range(N):
            for c in range(N):
                ...
                # підсвічування плитки з максимальним значенням рамкою
                if v == max_val and v > 0:
                    # use Border object; helper functions may vary by flet version
                    try:
                        tile.border = ft.Border.all(3, ft.Colors.AMBER_700)
                    except Exception:
                        # fallback if Border.all is not available
                        try:
                            tile.border = ft.border.Border(width=3, color=ft.Colors.AMBER_700)
                        except Exception:
                            tile.border = ft.Border(width=3, color=ft.Colors.AMBER_700)
                else:
                    tile.border = None

        # Оновлення прогрес-бару (логарифмічна шкала)
        if max_val > 1:
            progress_bar.value = math.log2(max_val) / WIN_STEPS
            progress_bar.value = max(0.0, min(1.0, progress_bar.value))
        else:
        ...
        if game.state == "won":
            ...
            status_text.value = msg or "Стрілки - хід. R - заново. Z - відміна ходу"
        page.update()

    # If there is a saved state, enable Continue button and optionally pre-load best_score
    if has_saved_state:
        # load best_score into game.best_score for UI consistency
        try:
            game.best_score = int(saved.get("best_score", game.best_score))
        except Exception:
            pass
    ...
    def on_restart(e: ft.ControlEvent) -> None:
        game.reset()
        save_state(game)
        refresh_ui("Стрілки - хід. R - заново. Z - відміна ходу")

    def on_continue(e: ft.ControlEvent) -> None:
        nonlocal saved, has_saved_state
        saved = load_state()
        if not saved:
        ...
        try:
            ...
            game.score = int(saved.get("score", game.score))
            game.moves = int(saved.get("moves", game.moves))
            game.best_score = int(saved.get("best_score", game.best_score))
            game.state = "playing"
            save_state(game)  # ensure file consistent
        except Exception:
            pass
        refresh_ui("Продовжено зі збереження")
        page.update()
    ...
    def apply_theme() -> None:
        ...
        for btn in [restart_btn, themes_btn, stats_btn, continue_btn]:
            btn.style = new_style
    ...
    restart_btn = ft.Button(content="🔁 New Game", on_click=on_restart, style=btn_style, width=145)
    stats_btn = ft.Button(content="📈 Stats", on_click=on_stats_click, style=btn_style, width=145)
    # Continue button: active only when saved state exists
    continue_btn = ft.Button(content="▶️ Продовжити", on_click=on_continue, style=btn_style, width=145)
    continue_btn.disabled = not has_saved_state

    themes_btn = ft.Button(content=theme()["icon"], on_click=on_theme_toggle, style=btn_style)
    ...
    def on_key(e: ft.KeyboardEvent) -> None:
        ...
        # Z — відміна ходу (аналог Backspace), тільки якщо гра не програна
        if e.key == "Z" and game.state != "lost":
            game.undo()
            save_state(game)
            refresh_ui()
            return

        # Backspace — відміна ходу
    ...
    header = ft.Row(
        controls=[
            title_text,
            ft.Column(
                controls=[restart_btn, stats_btn, continue_btn],
